...
public class Application {
	...
	public static void main( String[] args) {
		...
		{
			int sum = accounts.stream().filter( a -> a.getCustomer() == c1).map( a -> a.getBalance())
		...
		{
			IntStream.range( 0, accounts.size()).mapToObj( i -> accounts.get( i)).filter( a -> a.getCustomer() == c1)
			.forEach( a -> System.out.println( a));
		}
		{
			accounts.stream().filter( a -> a.getCustomer() == c1).forEach( a -> System.out.println( a));
		}
